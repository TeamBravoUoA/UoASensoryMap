...
(function () {
  ...
  function renderSensory(profiles) {
    ...
    wrap.innerHTML = profiles
      .map(
        (p) =>
          '<div class="sensory-line"><span class="s-label">' +
          escapeHtml(p.attribute) +
          '</span><span class="s-bar"><b style="width:' +
          (p.rating / 5) * 100 +
          "%;background:" +
          scaleColour(p.rating) +
          '"></b></span><span class="val">' +
          p.rating +
          "/5</span></div>"
      )
      .join("");
    ...
    wrap.innerHTML = spaces
      .map((s) => {
        ...
        return (
          ...
          '<header><span class="space-type-badge">' +
          escapeHtml(s.space_type_display || s.space_type) +
          "</span><h4>" +
          escapeHtml(s.name) +
          "</h4>" +
          flags +
  ...
  function renderGallery(images) {
    ...
    wrap.innerHTML = images
      .map(
        (img) =>
          '<a href="' + escapeHtml(img.image) + '" target="_blank" rel="noopener"><img src="' +
          escapeHtml(img.thumbnail || img.image) +
          '" alt="' + escapeHtml(img.alt_text || "Gallery image") + '" loading="lazy"></a>'
      )
      .join("");
  }
  ...
  async function loadDetail() {
    try {
      ...
      el("dp-category").textContent = d.category_display || categoryLabel(d.category);
      el("dp-sub").textContent =
        (d.category_display || categoryLabel(d.category)) +
        (d.campus_display ? " \u00B7 " + d.campus_display : "");
      el("dp-also").textContent = d.also_known_as ? "Also known as " + d.also_known_as : "";
      el("dp-desc").textContent = d.description || "No description provided.";
      ...
      el("detail-error").textContent = "Could not load place details: " + err.message;
    }
  }
